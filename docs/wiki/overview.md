...
`unified_train_pipeline` 是一个配置驱动的深度学习模型训练框架。它通过统一的 `data_dict` 在数据加载、模型前向、损失计算与 hook 阶段之间传递状态，并通过 registry 机制实现 dataset/model/loss 的解耦构建，支持单机与 DDP 分布式训练。

## 架构

整体执行路径为：`scripts/train.py` 解析配置并触发 `tasks` 自动注册，然后创建 `Trainer` 完成数据集、模型、损失与优化器构建，最后进入 epoch/batch 训练循环。模型层默认由 `UnifiedModel` 串联子模块，loss 可由独立模块计算或从模型输出读取，checkpoint 通过 `CheckpointIO` 抽象保存（当前实现为本地文件系统）。
...
├── registry/    - dataset/model 构建注册器与自动导入工具
├── hooks/       - hook 协议和执行链
├── interfaces/  - checkpoint 与预测格式接口定义
├── train/       - Trainer、训练 loop 工具、DDP 工具
├── tasks/       - 各任务的 dataset/model/loss 插件实现
...
- Registry + decorator 注册（`MODULE_REGISTER`, `DATASET_REGISTER`）
- 共享可变 `data_dict` 流水线
